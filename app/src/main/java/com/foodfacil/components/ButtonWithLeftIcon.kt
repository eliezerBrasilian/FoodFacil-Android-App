    ...
    Box(
        ...
        Button(onClick = onClick, colors = ButtonDefaults.buttonColors(background),
            ...
            Box(contentAlignment = Alignment.Center){
                if(isLoading) CircularProgressIndicator(md.size(progressIndicatorSize), color = progressIndicatorColor)
                    ...
                    Text(text = text,fontSize = fontSize, color = textColor, style = fontStyle, lineHeight = 1.sp)
                }
            }
        }
    }
        ...
        Box(
            md
                ...
                .padding(horizontal = marginHorizontal)){
            Button(onClick = onClick, colors = ButtonDefaults.buttonColors(background),
                modifier = md.fillMaxWidth().height(50.dp), border = BorderStroke(width = borderWidth, color = borderColor,)
            ){
                Box(contentAlignment = Alignment.Center){
                    if(isLoading) CircularProgressIndicator(md.size(progressIndicatorSize), color = progressIndicatorColor)
                        else { Row(horizontalArrangement = Arrangement.spacedBy(10.dp), verticalAlignment = Alignment.CenterVertically) {
